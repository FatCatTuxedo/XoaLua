...
function plantStingerCommand()
	if (getTeamName(getPlayerTeam(getLocalPlayer())) == "Law Enforcement") then
...
addCommandHandler("stinger", plantStingerCommand)

function onClientRender()
	if ( isPedInVehicle( localPlayer ) ) then--and ( getPlayerWantedLevel() >= 1 ) then
		...
		for k, v in ipairs(g_Stingers) do
			if getElementModel(v) == 2899 then
				...
				if getDistanceBetweenPoints3D(wx1, wy1, wz1, vx, vy, vz) <= 1.4 then
					--setVehicleWheelStates(vehicle, 1, -1, -1, -1)
					setElementAngularVelocity ( vehicle, 0, 0, 0.2 )
					if (v) then
						triggerServerEvent("onDestroyStinger", getLocalPlayer(), v)
					end
				end
				if getDistanceBetweenPoints3D(wx2, wy2, wz2, vx, vy, vz) <= 1.4 then
					--setVehicleWheelStates(vehicle, -1, 1, -1, -1)
					setElementAngularVelocity ( vehicle, 0, 0, 0.2 )
					if (v) then
						triggerServerEvent("onDestroyStinger", getLocalPlayer(), v)
				...
				if getDistanceBetweenPoints3D(wx3, wy3, wz3, vx, vy, vz) <= 1.4 then
					--setVehicleWheelStates(vehicle, -1, -1, 1, -1)
					setElementAngularVelocity ( vehicle, 0, 0, 0.2 )
					if (v) then
						triggerServerEvent("onDestroyStinger", getLocalPlayer(), v)
				...
				if getDistanceBetweenPoints3D(wx4, wy4, wz4, vx, vy, vz) <= 1.4 then
					--setVehicleWheelStates(vehicle, -1, -1, -1, 1)
					setElementAngularVelocity ( vehicle, 0, 0, 0.2 )
					if (v) then
						triggerServerEvent("onDestroyStinger", getLocalPlayer(), v)
